        ...
        <div style="background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 0.75rem; padding: 1.5rem; margin-bottom: 2rem; margin-left: 1rem; margin-right: 1rem;" class="shadow-sm">
            <form method="GET" action="{{ route('phones.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    ...
                    <input type="number" name="year" value="{{ request('year') }}" placeholder="Напр. 2023" style="width: 100%; border: 1px solid #d1d5db; border-radius: 0.5rem; padding: 0.5rem;" class="shadow-sm">
                </div>
                <div>
                    ...
                    <select name="model_id" style="width: 100%; border: 1px solid #d1d5db; border-radius: 0.5rem; padding: 0.5rem;" class="shadow-sm">
                        ...
                        @foreach($models as $model)
                            <option value="{{ $model->id }}" @selected(request('model_id') == $model->id)>{{ $model->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="manufacturer_id" style="width: 100%; border: 1px solid #d1d5db; border-radius: 0.5rem; padding: 0.5rem;" class="shadow-sm">
                        ...
                        @foreach($manufacturers as $manufacturer)
                            <option value="{{ $manufacturer->id }}" @selected(request('manufacturer_id') == $manufacturer->id)>{{ $manufacturer->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex gap-2">
                    <button type="submit" style="background-color: #2563eb; color: white; border-radius: 0.5rem; padding: 0.5rem 1rem; font-weight: 500; border: none; cursor: pointer;" class="flex-1 hover:opacity-90 shadow-sm">Търси</button>
                    <a href="{{ route('phones.index') }}" style="background-color: #ffffff; border: 1px solid #d1d5db; color: #374151; border-radius: 0.5rem; padding: 0.5rem 1rem; text-decoration: none; text-align: center;">Изчисти</a>
                </div>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead style="background-color: #f8fafc;">
                        <tr>
                            <th scope="col" style="color: #64748b; padding: 1rem 1.5rem; text-align: left; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Снимка</th>
                            <th scope="col" style="color: #64748b; padding: 1rem 1.5rem; text-align: left; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Име на устройство</th>
                            <th scope="col" style="color: #64748b; padding: 1rem 1.5rem; text-align: left; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Серия / Модел</th>
                            <th scope="col" style="color: #64748b; padding: 1rem 1.5rem; text-align: left; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Марка</th>
                            <th scope="col" style="color: #64748b; padding: 1rem 1.5rem; text-align: left; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">Година</th>
                    ...
                    <tbody style="background-color: #ffffff;" class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td style="padding: 1rem 1.5rem; white-space: nowrap;">
                                @if($phone->image)
                                    <img src="{{ asset('storage/' . $phone->image) }}" alt="Phone" style="width: 50px; height: 50px; object-fit: contain; border-radius: 4px; border: 1px solid #eee;">
                                @else
                                    <div style="width: 50px; height: 50px; background-color: #f3f4f6; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: #9ca3af; font-size: 0.7rem;">Няма</div>
                                @endif
                            </td>
                            <td style="padding: 1rem 1.5rem; white-space: nowrap;">
                        ...
                        @empty
                        <tr>
                            <td colspan="6" style="padding: 2.5rem; text-align: center; color: #6b7280;">
