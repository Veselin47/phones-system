...
<div style="background-color: #f3f4f6; min-height: 100vh; padding-top: 3rem; padding-bottom: 3rem;">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <h1 style="color: #1f2937; font-size: 1.875rem; font-weight: 800; margin-top: 0.5rem;">Редактирай телефон</h1>
        </div>
        ...
        <div style="background-color: white; padding: 2rem; border-radius: 0.75rem; border: 1px solid #e5e7eb; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1);">
            ...
            <form action="{{ route('admin.phones.update', $phone->id) }}" method="POST">
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <select name="phone_model_id" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.5rem;" required>
                        @foreach($models as $model)
                            <option value="{{ $model->id }}" {{ $model->id == $phone->phone_model_id ? 'selected' : '' }}>
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <select name="manufacturer_id" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.5rem;" required>
                        @foreach($manufacturers as $manufacturer)
                            <option value="{{ $manufacturer->id }}" {{ $manufacturer->id == $phone->manufacturer_id ? 'selected' : '' }}>
                                {{ $manufacturer->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div style="margin-bottom: 2rem;">
                    <label style="display: block; font-weight: 600; color: #374151; margin-bottom: 0.5rem;">Година</label>
                    <input type="number" name="release_year" value="{{ $phone->release_year }}" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.5rem;">
                </div>
